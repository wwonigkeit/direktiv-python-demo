...
class DirektivHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if all(key in reqData for key in ('searchstring', 'bearertoken', 'projectid','location','gcpkey','maxsearchreturns','outputfile')):
            ...
            for tweet in tweets:
                language, confidence = self._detect_language(tweet.text,reqData['location'],reqData['projectid'],reqData['gcpkey'],actionID)
            ...
            self._log(actionID, "Completed twitter scrape and language detection")
